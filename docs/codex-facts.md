...
- лишняя (устаревшая) запись — это не просто шум: она блокирует промпт сама по себе. Поэтому `merge_codex_hooks` наши прежние записи вытесняет, а не дополняет (`lib/setup_helpers.py`, `OUR_HOOK_SCRIPTS`);
