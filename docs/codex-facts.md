...
#### Чем лечится: прогрев на установке (проверено замером и живым запуском 10.08.2026)

Замеры на чистом `HOME` (пустой кэш `uv`), время до готовности MCP-сервера отвечать на `initialize`:

| коннектор | холодный кэш | после прогрева |
|---|---|---|
| `connectors/trino_proxy.py` | **17,7 с** | 0,4 с |
| `connectors/sheets_mcp.py` | 2,5 с | — |

Одного `uv sync --script <файл>` НЕ хватает: он занимает 6,2 с и наполняет кэш пакетов, но первый `uv run` после него всё равно шёл **12,0 с** — окружение самого скрипта собирается только на запуске. Поэтому прогрев в мастере делает и то, и другое: `uv sync --script` (по его коду возврата видно, доехали ли зависимости — у `uv run` код принадлежит скрипту) и `uv run <файл> </dev/null` (MCP-сервер завершается на EOF сам).

Живая проверка результата: чистая песочница (свой `HOME`, свой `CODEX_HOME`, клон репозитория), полный `./setup.sh`, затем ПЕРВАЯ сессия — оба локальных коннектора отвечают настоящими списками инструментов (`trino`: `describe_table`, `execute_query`, `list_catalogs`, `list_schemas`, `list_tables`; `clickhouse-wms` через обёртку `clickhouse_proxy.py`: `list_databases`, `list_tables`, `run_query`). Для сравнения — тот же клон без прогрева, на холодном кэше: инструментов `trino` в сессии нет вовсе.

`startup_timeout_sec` так и остался непроверенным: прогрев убирает причину, а не симптом, и вводить непроверенную настройку «на всякий случай» не стал.

**Для дизайна:** первая сессия после установки — самая опасная, ровно там связка «всё настроено, а ничего нет» и проявляется. Сторонние серверы под `uvx`/`npx` (`atlassian`, `grafana`, `openmetadata`, `growthbook`) на холодном кэше в живых проверках успевали стартовать (`atlassian` отдал 98 инструментов на первом же запуске в чистой песочнице), поэтому их прогрев не делаю — но это наблюдение, а не гарантия.
...
### У самого Codex ДВА формата транскрипта: `codex exec` и интерактивный TUI (проверено живым запуском 10.08.2026)

Найдено по живым данным телеметрии: у всех сессий Codex `n_prompts` был 0 при непустых `n_tools` и токенах, хотя события `UserPromptSubmit` от тех же сессий в `sandbox.ai_usage_events` лежали.

Причина — промпт человека записывается **по-разному в зависимости от того, чем запущен Codex**, на одной и той же версии (`cli_version: 0.147.0` в обоих файлах). Различаются они полем `session_meta.payload.originator`:

| запуск | `originator` | `source` | запись промпта |
|---|---|---|---|
| `codex exec` | `codex_exec` | `exec` | `event_msg` / `payload.type = "user_message"`, текст строкой в `payload.message` |
| интерактивный `codex` (TUI) | `codex-tui` | `cli` | `event_msg` / `payload.type = "item_completed"`, `payload.item.type = "UserMessage"`, текст — список блоков в `item.content` |

Снято на четырёх живых файлах: два `exec` (в т.ч. проверка доверия хукам из мастера установки) и два TUI (сессия приёмки и специально снятая сессия под псевдотерминалом). Ни в одном TUI-файле нет ни одной записи `user_message`, ни в одном exec-файле — ни одной `item_completed`.

Блоки `item.content` у TUI-промпта:
```json
[{"type": "text", "text": "$task OE-3491", "text_elements": []},
 {"type": "skill", "name": "task", "path": "/…/.agents/skills/task/SKILL.md"}]
```
То есть вызванный скилл размечен структурно — в отличие от `codex exec`, где скилл приходилось угадывать по пути `SKILL.md` в аргументах вызова инструмента.

Соседние типы `item` (перечень видно в самом бинаре рядом с `UserMessage`): `HookPrompt`, `AgentMessage`, `Reasoning`, `DynamicToolCall`, `CollabAgentToolCall`, `WebSearch`, `ImageView`, `Extension`, `ImageGeneration`, `EnteredReviewMode`, `McpToolCall`, `ContextCompaction`. Важное следствие: **промпт, вставленный хуком, — это отдельный тип `HookPrompt`**, а не `UserMessage`, поэтому наше сообщение SessionStart про `git pull` не попадает в счёт промптов человека структурно, а не по счастливой случайности.

**Урок, который дороже самого факта:** все живые проверки этого репозитория до сих пор шли через `codex exec` — им удобно управлять из скрипта. Аналитик при этом работает в TUI. Проверка «живым запуском» в режиме, которым человек не пользуется, — это всё ещё проверка на заглушке, просто дорогой. TUI гоняется через псевдотерминал (`pty.openpty`), это рабочий способ, им и сняты факты выше.

### `SessionEnd` у Codex: причина всегда `other`, словаря причин нет (проверено живым запуском)

У Claude Code `reason` в `SessionEnd` осмысленный — на живых данных встречались `prompt_input_exit`, `clear`, `resume`. У Codex во всех наблюдениях — только `"other"`:

- `codex exec` — `{"hook_event_name": "SessionEnd", "reason": "other"}` (раздел 2);
- интерактивный TUI, выход по Ctrl-D — тот же `reason: "other"` (снято под псевдотерминалом, полная последовательность событий сессии: `SessionStart(source=startup)` → `UserPromptSubmit` → `Stop` → `SessionEnd(reason=other)`).

Плюс статическая проверка бинаря: строки `prompt_input_exit` в нём **нет вообще** (`strings | grep -c` → 0), отдельного enum вроде `SessionEndReason` тоже не нашёл. Значения `startup`/`resume`/`clear`/`compact`, которые лежат в бинаре рядом с `SessionEnd`, — это `source` у `SessionStart`, а не причины завершения.

**Вывод для дизайна:** приводить `end_reason` Codex «к тому же смыслу», что у Claude Code, не из чего. Мы пишем то, что прислал движок, и не подставляем ничего своего: выводить причину из транскрипта (например, считать `exit` в последнем промпте выходом) — это выдумывание данных, которых движок не давал. Сравнивать движки по `end_reason` нельзя, и это должно быть видно по данным.
