...
Инструменты видны как `mcp__superset__<name>`. Полный список: в Claude Code —
команда `/mcp`; в Codex такой команды нет (`codex mcp list` наш профиль не
видит), там смотри сам исходник — `connectors/superset_mcp.py`.
...
Для сырых данных без привязки к объектам Superset — витрины через
`clickhouse-wms` / `clickhouse-dwh` / `trino` (какой кластер — см. скилл
`clickhouse-sql`, раздел «Два кластера, два коннектора», и колонку «Кластер»
в `context/marts.md`); коннектор `superset` — когда вопрос именно про
объекты Superset.

## Проверка реестра — обязательно до первого чтения

Прежде чем звать инструмент, который читает конкретный дашборд/чарт —
`get_dashboard`, `get_chart_data`, `get_dashboard_layout_summary`, любой
скриншот, `sql_query` — сверься с `context/dashboards.md`. Дашборд помечен
«🚫 запрещено — правило 1» — останавливаешься сразу: ничего из перечисленного
не зовёшь, говоришь человеку, что дашборд зарплатный и почему (правило 1,
`CLAUDE.md`). Discover/inspect-инструменты вроде `list_dashboards`,
`list_charts`, `list_datasets` безопасны без этой сверки — они возвращают
только метаданные (название, id, владельцы), не данные под дашбордом.
