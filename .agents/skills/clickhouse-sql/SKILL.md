...
## Два кластера, два коннектора — выбери правильный

ClickHouse — это два независимых кластера с разными учётками, а не один
источник с двумя адресами. Коннекторы называются явно: `clickhouse-wms` и
`clickhouse-dwh`. Не перебирай оба наугад — ошибка запроса на неверном
кластере выглядит как «нет такой базы», а не как понятная подсказка.

- **`clickhouse-wms`** (`mcp__clickhouse-wms__*`) — складской кластер: WMS,
  операционная аналитика, склады, сборка/приёмка/доставка. Здесь же
  телеметрия (`sandbox.ai_usage_*`).
- **`clickhouse-dwh`** (`mcp__clickhouse-dwh__*`) — общий кластер: продажи,
  финансы, маркетинг.

Как выбрать:
1. Смотришь на конкретную витрину/таблицу — сверься с колонкой «Кластер» в
   `context/marts.md` (см. раздел ниже). Там написано `WMS`, `DWH`,
   `WMS, DWH` (есть на обоих — независимые таблицы с совпавшим именем, а не
   одна витрина) или отдельная пометка для «не найдена»/«не ClickHouse».
2. Витрины в реестре нет вообще, или колонка «Кластер» неоднозначна для
   задачи — не гадай и не гоняй один и тот же запрос на оба коннектора по
   очереди: спроси человека, какой кластер имеется в виду.
3. Общее правило по смыслу задачи, если реестр не помог: складские/операционные
   данные — `clickhouse-wms`; продажи/финансы/маркетинг — `clickhouse-dwh`.
...
1. Write the query
2. Test via the `trino`, `clickhouse-wms` or `clickhouse-dwh` MCP-коннектор
   (см. «Два кластера, два коннектора» выше — не гадай, какой из двух)
