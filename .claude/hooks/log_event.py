...
def main():
    try:
        ...
        if row and row["engine"] == hook_payload.ENGINE_CODEX \
                and payload.get("hook_event_name") == "UserPromptSubmit":
            # Обогащение сессии Codex: токены, модель, скиллы, jira_key — то,
            # что живёт только в строке сессии и раньше приезжало лишь на
            # SessionEnd, то есть при чистом выходе, которого обычно нет.
            #
            # Почему именно здесь, а не отдельным хуком Stop, как у Claude
            # Code: Codex регистрирует хуки в $CODEX_HOME/hooks.json — файле
            # ВНЕ репозитория, который `git pull` на SessionStart не
            # обновляет. Новое событие в нём означало бы обход всех машин
            # аналитиков с переустановкой; UserPromptSubmit там уже
            # зарегистрирован и доедет сам (lib/setup_helpers.py,
            # CODEX_HOOK_EVENTS).
            #
            # Плата за это — строка отражает состояние ДО текущего хода:
            # промпт только отправлен, ответа ещё нет. То есть агрегаты
            # отстают на один ход, а последний ход сессии без чистого выхода
            # не попадёт вовсе. Это заметно меньшая потеря, чем сессия
            # целиком, и она видна в данных: n_prompts из событий (их пишет
            # эта же функция строкой ниже) будет на единицу больше, чем из
            # строки сессии.
            #
            # Импорт внутри ветки, а не наверху файла: этот хук висит на
            # КАЖДОМ вызове инструмента, а сюда попадает только промпт Codex.
            import log_session  # noqa: E402  (сосед по каталогу хуков)
            log_session.log_progress(payload)
        if row:
