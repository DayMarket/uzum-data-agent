#!/usr/bin/env python3
"""Разбор и рендер реестра витрин.

Это библиотека, а не команда: у неё намеренно нет `__main__`. Раньше он был и
рендерил реестр из пустого списка строк — то есть первый, кто выполнил бы
обещание из шапки файла буквально (`python3 tools/build_context.py
context/marts.md > context/marts.md`), обнулил бы реестр вместе с ручными
колонками. Источника строк у генератора нет: реестры собраны разовой
выгрузкой из Superset API (см. шапку context/marts.md) и дальше живут руками.

Здесь два куска, которые понадобятся при следующей пересборке:
  read_trust(path)          — вынуть из существующего файла ручные колонки
                              «доверие» и «комментарий», чтобы не потерять их;
  render_marts(rows, trust) — собрать таблицу заново, вернув ручные колонки
                              на место.
"""
import sys
...
def read_trust(path):
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        # Разделитель шапки. Двоеточия — это выравнивание колонки в markdown
        # (|---|--:|), без них строка |---|--:|---| не распознавалась как
        # разделитель и разбор молча возвращал пустой словарь: ручные колонки
        # реестра терялись бы при первой же пересборке.
