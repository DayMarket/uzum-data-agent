...
# значение: вписать новое в .env либо переподключить один коннектор через
# ./setup.sh --add <коннектор> — там вопрос задаётся всегда.
#
...
живым запросом. Поменять сохранённое значение: вписать новое в .env либо
./setup.sh --add <коннектор>, там вопрос задаётся всегда.
...
# Сохранённые доступы — только в полном прогоне. `./setup.sh --add <коннектор>`
# существует ровно для того, чтобы ввести значение заново (протух токен,
# сменился пароль), и подстановка сохранённого лишила бы человека
# единственного способа его поменять: вопрос бы не задали, а живая проверка
