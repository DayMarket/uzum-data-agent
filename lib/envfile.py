...
отсутствие переменной оставляет коннектор без значения. (Здесь раньше
стояло «валит запись MCP-сервера целиком» — на текущей версии Claude Code
это не так: сервер стартует, а плейсхолдер уезжает дочернему процессу
буквально, строкой `${VAR}` — проверено живым запуском 14.08.2026. Оба
исхода лечит мостик connectors/claude_env_bridge.py, который добирает
значения из этого же файла.)
